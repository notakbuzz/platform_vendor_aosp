...
func init() {
	android.RegisterModuleType("cyclone_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cycloneExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cycloneExpandVariables(ctx, dep_file)
	...
	cmd := cycloneExpandVariables(ctx, String(g.properties.Cmd))
